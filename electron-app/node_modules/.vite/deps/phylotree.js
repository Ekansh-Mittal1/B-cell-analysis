  ...
  "node_modules/lodash/lodash.js"(exports2, module) {
    (function() {
      ...
      var runInContext = function runInContext2(context) {
        ...
        var DataView2 = getNative(context, "DataView"), Map2 = getNative(context, "Map"), Promise2 = getNative(context, "Promise"), Set2 = getNative(context, "Set"), WeakMap = getNative(context, "WeakMap"), nativeCreate2 = getNative(Object2, "create");
...
function getTaggedNewick(options = {}) {
  const tag = options.tag || "Foreground";
  const multiSet = options.multiSet || false;
  const selectionAttr = this.display ? this.display.selection_attribute_name : "selected";
...
function drawNode(container, node, transitions) {
  ...
  if (is_leaf || this.showInternalName(node) && !isNodeCollapsed(node)) {
    labels = labels.enter().append("text").classed(this.css_classes["node_text"], true).merge(labels).on("click", (event, d) => {
...
function nodeDropdownMenu(node, container, phylotree, options, event) {
  ...
  if (menu_object.empty()) {
    menu_object = select_default(container).append("div").attr("id", d3_layout_phylotree_context_menu_id).attr("class", "phylotree-context-menu dropdown-menu").attr("role", "menu");
  }
  ...
  if (node) {
    ...
    if (!isLeafNode(node)) {
      if (options["collapsible"]) {
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text(isNodeCollapsed(node) ? "Expand Subtree" : "Collapse Subtree").on("click", (d) => {
          menu_object.style("display", "none");
          this.toggleCollapse(node).update();
        });
        if (options["selectable"]) {
          menu_object.append("div").attr("class", "phylotree-menu-divider dropdown-divider");
          menu_object.append("h6").attr("class", "phylotree-menu-header dropdown-header").text("Toggle selection");
        }
      }
      if (options["selectable"]) {
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text("All descendant branches").on("click", (d) => {
          menu_object.style("display", "none");
        ...
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text("All terminal branches").on("click", (d) => {
          menu_object.style("display", "none");
        ...
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text("All internal branches").on("click", (d) => {
          menu_object.style("display", "none");
    ...
    if (node.parent) {
      if (options["selectable"]) {
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text("Incident branch").on("click", (d) => {
          menu_object.style("display", "none");
          if (this.options["selection-mode"] === "multi-set" && this._activeSetName) {
        ...
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text("Path to root").on("click", (d) => {
          menu_object.style("display", "none");
      ...
      if (options["reroot"]) {
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text("Reroot on this node").on("click", (d) => {
          menu_object.style("display", "none");
      ...
      if (options["hide"]) {
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text("Hide this " + (isLeafNode(node) ? "node" : "subtree")).on("click", (d) => {
          menu_object.style("display", "none");
    ...
    if (hasHiddenNodes(node)) {
      menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text("Show all descendant nodes").on("click", function(d) {
        menu_object.style("display", "none");
    ...
    if (has_user_elements.length) {
      ...
      has_user_elements.forEach(function(d) {
        menu_object.append("a").attr("class", "phylotree-menu-item dropdown-item").attr("tabindex", "-1").text(d[0](node)).on("click", partial_default(d[1], node));
      });
    }
...
function selectNodes(names) {
  if (!names || !Array.isArray(names) || names.length === 0) {
    return this;
  }
  const nodesToSelect = this.phylotree.nodes.descendants().filter((node) => names.includes(node.data.name));
  if (nodesToSelect.length > 0) {
    this.modifySelection(nodesToSelect, this.selection_attribute_name, false, false, "true");
  }
  return this;
}
function deselectNodes(names) {
  if (!names || !Array.isArray(names) || names.length === 0) {
    return this;
  }
  const nodesToDeselect = this.phylotree.nodes.descendants().filter((node) => names.includes(node.data.name));
...
// node_modules/phylotree/src/render/event-emitter.js
...
function off(eventName, callback) {
  if (this._eventListeners && this._eventListeners[eventName]) {
    this._eventListeners[eventName] = this._eventListeners[eventName].filter((cb2) => cb2 !== callback);
  }
  return this;
}
...
// node_modules/phylotree/src/render/selection-sets.js
var selection_sets_exports = {};
__export(selection_sets_exports, {
  _getSetsSummary: () => _getSetsSummary,
  _injectSetStyles: () => _injectSetStyles,
  addToSet: () => addToSet,
  getActiveSet: () => getActiveSet,
  getNodeSet: () => getNodeSet,
  getSetMembers: () => getSetMembers,
  handleMultiSetClick: () => handleMultiSetClick,
  initializeSelectionSets: () => initializeSelectionSets,
  isInSet: () => isInSet,
  removeFromSet: () => removeFromSet,
  setActiveSet: () => setActiveSet
});
function initializeSelectionSets(sets) {
  this._selectionSets = {};
...
function _injectSetStyles(sets) {
...
function getSetMembers(setName) {
  if (!this._selectionSets || !this._selectionSets[setName]) {
    return [];
  }
  const memberNames = this._selectionSets[setName].members;
  return this.phylotree.nodes.descendants().filter((node) => memberNames.has(node.data.name));
}
function addToSet(node, setName) {
  if (!this._selectionSets || !this._selectionSets[setName]) {
    console.warn(`Selection set '${setName}' not found`);
    return this;
  }
  const nodeName = typeof node === "string" ? node : node.data.name;
  const nodeObj = typeof node === "string" ? this.phylotree.getNodeByName(node) : node;
  if (nodeObj) {
...
function removeFromSet(node, setName) {
  if (!this._selectionSets || !this._selectionSets[setName]) {
    return this;
  }
  const nodeName = typeof node === "string" ? node : node.data.name;
...
function handleMultiSetClick(node) {
...
var TreeRender = class {
  constructor(phylotree, options = {}) {
    ...
    if (this.options["selection-mode"] === "multi-set" && this.options["selection-sets"].length > 0) {
  ...
  applyInitialSelection() {
    if (this.options["initial-selection"] && this.options["initial-selection"].length > 0) {
      this.selectNodes(this.options["initial-selection"]);
    }
    if (this.options["selection-mode"] === "multi-set" && this.options["initial-sets"]) {
  ...
  initialize_svg(svg_element) {
    if (this.svg !== svg_element) {
      ...
      if (this.options["responsive"]) {
        this.svg.attr("viewBox", `0 0 ${this.width} ${this.height}`).attr("preserveAspectRatio", this.options["preserve-aspect-ratio"]).style("width", "100%").style("height", "auto").style("max-width", "100%");
      } else {
...
extend_default(TreeRender.prototype, event_emitter_exports);
extend_default(TreeRender.prototype, selection_sets_exports);
